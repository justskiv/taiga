...
- ПАРИТЕТ вывода: снял baseline до правок. Страницы рубрик/статей —
  ...
  404 отличаются от baseline ТОЛЬКО флипом active-класса у пункта
  «Лаборатория» — это предсуществующий недетерминизм кэша шапки (см.
  найденный баг ниже), НЕ следствие правок 2.5 (флипает и без них,
  завязан на `.Section=""`).
...
- ⚠ НАЙДЕН БАГ (предсуществующий, с ф.01; НЕ чинил — вне scope 2.5,
  территория ф.03 «Хром»; жду решения пользователя): активный пункт
  навигации на страницах БЕЗ секции недетерминирован. `baseof.html`
  кэширует шапку `partialCached "header.html" . .Section`; home, 404
  и /lab имеют `.Section == ""` и делят ОДИН кэш. Кто отрендерится
  первым (конкурентно, случайно), тот задаёт active-состояние всем
  троим: /lab первым → «Лаборатория» class=on протекает на home и
  404; home первым → /lab ТЕРЯЕТ свой active. Проверено 4 пересборки:
  состояние флипает (build 1-3 off, build 4 on), троица всегда
  согласована — доказывает коллизию по ключу "". Статьи рубрик
  (.Section!="") не затронуты (их паритет байт-в-байт держится).
  Предлагаемый фикс (1 строка): ключ кэша `(or .Section .RelPermalink)`
  — секции делят кэш как сейчас, бессекционные страницы кэшируются
  поштучно (проверить, что у 404 .RelPermalink уникален).
