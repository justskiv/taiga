...
| `home.hero` | `"wordmark"` \| `""` | `"wordmark"` | Home heading scene above the kicker: the header brand (logo + name) writ large, the kicker demoted to its centered subtitle. A site may add a mascot beside the mark by shipping `_partials/home/mascot.html`. `""` ⇒ no scene, a plain column. See [customizing.md](customizing.md#home-look). |
| `home.grid` | `"grid"` \| `"fade"` \| `"dots"` \| `""` | `"fade"` | Background markup behind the home hero zone, drawn at the threshold of visibility in the active palette's own ink; `"fade"` dissolves it toward the feed. `""` ⇒ plain background. |
| `home.rubricCards` | `"boxed"` \| `"naked"` | `"naked"` | Rubric card chrome: `"naked"` drops the box (fill + border) and grows the logo to 112px — free columns on the page, and what lets the hero backdrop read between the tiles. `"boxed"` is the framed card, for a home with no backdrop. |
...
| `article.cover` | bool | `false` | Prints the guide's `cover:` picture above the article prose — WHOLE, while the feed band crops the same file. Unset ⇒ nothing above the lead. See [authoring.md](authoring.md#covers-og-images). |
